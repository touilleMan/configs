...
try:
    makedirs = getattr(os, 'makedirs') or os.path.makedirs
    makedirs(TEMP_DIR)
...
# on a un gros array numpy qu'on a pas envie de se faire chier à se recréer
class Store(object):
...
# dossier. 
store = Store(os.path.join(TEMP_DIR, 'store'))
